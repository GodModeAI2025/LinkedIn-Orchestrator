...
# Prozentwerte in der Rangliste der Landingpage. Sie standen dort rund 120
# Zeichen Markup von ihrem Label entfernt, weiter als die Luecke von 40 Zeichen,
# die ein Kontextmuster ueberbruecken kann. Ein Muster ohne Kontext ginge nicht:
# '15 %' steht auf derselben Seite legitim als Gewicht der dritten Themen-Lane.
# Deshalb steht die Regel hier im Code und nicht in der Tabelle: die Zelle
# rank-pct traegt einen Rang, keine Zahl.
RANGLISTEN_SPALTE = re.compile(r'rank-pct">([^<]*)<')
...
def pruefe_rangliste(name: str, inhalt: str, fehler):
    """Die Rangliste der Landingpage traegt einen Rang, keine Zahl.

    Die Prozentspalte des Format-Rankings stand im Markup rund 120 Zeichen von
    ihrem Label entfernt. Ein Kontextmuster aus der Tabelle ueberbrueckt nur
    40 Zeichen, und ein Muster ohne Kontext ginge nicht: '15 %' steht auf
    derselben Seite legitim als Gewicht der dritten Themen-Lane. Deshalb steht
    diese Regel im Code. Sie prueft eine Stelle, nicht eine Aussage: wer den
    Prozentwert an einer anderen Stelle der Seite unterbringt, faellt ihr nicht
    auf.
    """
    for treffer in RANGLISTEN_SPALTE.finditer(inhalt):
        wert = treffer.group(1)
        if any(zeichen.isdigit() for zeichen in wert):
            zeile = inhalt.count("\n", 0, treffer.start()) + 1
            fehler.append(
                f"{name}:{zeile}: Die Spalte rank-pct traegt den Wert {wert!r}. "
                "Die Prozentwerte des Format-Rankings sind zurueckgezogen, die "
                "Spalte traegt einen Rang. Siehe references/SOURCES.md."
            )
...
def main() -> int:
    ...
    for name in GEPRUEFTE_DATEIEN:
        ...
        pruefe_rangliste(name, inhalt, fehler)
    ...
    for regel in BEHAUPTUNGS_REGELN:
        print(f"    - {regel['name']}")
    print("  1 Coderegel geprueft: rank-pct traegt einen Rang, keine Zahl")
